...
API_BASE="https://api-leak-osint-by-danxy-official-d4.vercel.app"
...
show_header() {
    echo -e "${DGREEN}┌─────────────────────────────────────────────────────────────────┐${NC}"
    echo -e "${DGREEN}│${WHITE}           DATA LEAK CHECKER OSINT BY DANXYTOOLS V8.4            ${DGREEN}│${NC}"
    echo -e "${DGREEN}├─────────────────────────────────────────────────────────────────┤${NC}"
    echo -e "${DGREEN}│${LGREEN}[i]${NC} Menggunakan API OSINT - Real-time Data                       ${DGREEN}│${NC}"
    echo -e "${DGREEN}│${LGREEN}[!]${NC} Maksimal 3 data ditampilkan                                  ${DGREEN}│${NC}"
    echo -e "${DGREEN}└─────────────────────────────────────────────────────────────────┘${NC}"
}
...
display_single_data() {
    ...
    sumber=$(echo "$data" | jq -r '.sumber // "N/A"')
    nama=$(echo "$data" | jq -r '.nama // "N/A"')
    nik=$(echo "$data" | jq -r '.nik // "N/A"')
    no_kk=$(echo "$data" | jq -r '.no_kk // "N/A"')
    alamat=$(echo "$data" | jq -r '.alamat // "N/A"')
    kecamatan=$(echo "$data" | jq -r '.kecamatan // "N/A"')
    nagari=$(echo "$data" | jq -r '.nagari // "N/A"')
    ...
    echo -e "${color}│${NC} NO KK      : ${WHITE}$no_kk${NC}"
    echo -e "${color}├─────────────────────────────────────────────────────────────────┤${NC}"
    echo -e "${color}│${NC} ALAMAT     : ${WHITE}$alamat${NC}"
    echo -e "${color}│${NC} KECAMATAN  : ${WHITE}$kecamatan${NC}"
    echo -e "${color}│${NC} NAGARI     : ${WHITE}$nagari${NC}"
    echo -e "${color}├─────────────────────────────────────────────────────────────────┤${NC}"
    echo -e "${color}│${NC} SUMBER     : ${YELLOW}$sumber${NC}"
    echo -e "${color}└─────────────────────────────────────────────────────────────────┘${NC}"
}
...
search_by_name_single() {
    clear_screen
    show_header
    echo -e "\n${YELLOW}[ SEARCH BY NAME - SINGLE RESULT ]${NC}"
    echo -e "${GRAY}─────────────────────────────────────────────────────────────────${NC}\n"
    
    echo -n "Masukkan Nama Lengkap (lebih spesifik): "
    read -r nama_input
    ...
    if [ "$http_code" -eq 200 ]; then
        ...
        if [ "$success" = "true" ]; then
            ...
            if [ $total -eq 1 ]; then
                # Jika hanya 1 data, tampilkan langsung
                data=$(echo "$content" | jq -r '.data[0]')
                display_single_data "$data" "" "${GREEN}"
            elif [ $total -gt 1 ]; then
                # Jika lebih dari 1, tampilkan 3 pertama dengan peringatan
                echo -e "${YELLOW}[!] Ditemukan $total data dengan nama yang mirip${NC}"
                echo -e "${YELLOW}[!] Menampilkan maksimal 3 data pertama${NC}\n"
                ...
                if [ $total -gt 3 ]; then
                    echo -e "\n${YELLOW}[!] Masih ada $((total-3)) data lain yang tidak ditampilkan${NC}"
                fi
            else
                echo -e "${RED}┌─────────────────────────────────────────────────────────────────┐${NC}"
                echo -e "${RED}│${WHITE}                      DATA TIDAK DITEMUKAN                       ${RED}│${NC}"
...
menu_leak_desa_seru() {
    while true; do
        ...
        echo -e "${DGREEN}│  ${GREEN}[3]${NC} CARI NAMA LENGKAP (Single/Specific)                        ${DGREEN}│${NC}"
